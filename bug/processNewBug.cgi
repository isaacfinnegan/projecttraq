...
my ( $connection, $q, %results, $newRecordId, $LOGGING, $DEBUG, $userid,
	%olddataref,$field,    %record );
my ( $cc, $g, $group, $units_req, $keywords, $word, $recordType, $rightnow );
...
else
{
	...
	no strict 'refs';
	my ($field);
	foreach $field ( split( ',', $c{bugtraq}{requiredforcreate} ) )
	{
		unless ( $record{$field} )
		{
			&log( "ERR: Required field $field not present", 5 );
			&doError( "$c{bugtraq}{label}{$field} must be set for this record", "", $q, "CreateBug" );
		}
	}
}
...
sub makeTemplate
{
    ...
    if ( $cgi->param('maketemplate') )
    {
        ...
        $projectid = $cgi->param('projectid');
        my ($templatename) = $cgi->param('templatename');
        &saveTemplate( $userid, $cgi, $templatename, $projectid, 'bug' );
        &log( "DEBUG: save template user: $userid, project:$projectid", 3 );
        &outputConfirmation( '', $cgi );
    }
...
sub outputConfirmation
{
    ...
    {
        print $q->redirect(
            "../actioncomplete.cgi?action=MakeTemplate&type=bug");
    }
    else
    {
        print $q->redirect(
"../actioncomplete.cgi?popup=$popup&id=$id&action=NewBug&result=Success&type=bug"
        );
    }
    &log( "New Bug Created: $newRecordId by $userid", 2 );
    &stopLog();
    exit 0;
}
##############################################################
