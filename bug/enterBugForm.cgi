...
&startLog();
$c{cache}{currenttype} = 'bug';
my ($LOGGING) = 5;

my ($tfile,
	$templatefile, $projectID, $connection, $q,   $projectid, %results, %recordToEdit, $userid, $i,
	%compmenu,     $recordid,  %milestones, $key, %ccs,       $type,    $product,      %owners
);
my (%html, %record_data,%res,$field,$fieldval,$fielddisp,$fieldoptionlist);

my ( $DEBUG, $db, $tt, $reporter, @groups, $html );
my ($q) = new CGI;
...
$html{COMMENTSIZE}[0]=$q->cookie('pt_bugcommentsize') || '150';
$html{NOTESIZE}[0]=$q->cookie('pt_bugnotesize') || '50';
$html{DUPLICATERESOLUTION}[0]=$c{bugtraq}{resolutionduplicate};
$DEBUG               = $q->param('debug');
$userid              = &getUserId($q);
...
{
	@requiredfields = split( ',', $c{bugtraq}{requiredforcreate} );
}
if ( $type eq 'edit' )
{
	@requiredfields = split( ',', $c{bugtraq}{requiredforedit} );
}
for ( my ($i) = 0 ; $i < scalar(@requiredfields) ; $i++ )
{
	...
	$html{'REQUIREDFIELDLABEL'}[$i] = $c{bugtraq}{label}{ $requiredfields[$i] };
...
if ( $type eq "new" || $type eq "template" )
{
	print $q->header;
	my ( $template, $prid );
	$record_data{long_desc}=$c{bugtraq}{recordentrytext};
	...
	{
		$html{SHORT_DESC_DISP}[0]="Choose a $c{bugtraq}{label}{projectid}..."; 
		$html{PROJECTID_OPTIONLIST}[0]="<option value=''>Choose a $c{bugtraq}{label}{projectid}...</option>\n" . &getFieldOptionList('projectid',\%record_data,$userid);
	}
	...
	%html = &populateLabels( \%html, 'bug' );
	# Use alternate template file if param is passed
	unless ($templatefile)
	{
		$tfile=&getTemplateFile($c{dir}{bugtemplates}, "enterBugTemplate.tmpl",$userid);
	}
	else
	{
		$tfile=&getTemplateFile($c{dir}{bugtemplates},$templatefile,$userid);
	}
}
elsif ( $type eq "edit" || $type eq "view" )
{
	...
	if ( $type eq "edit" )
	{
		$tfile=&getTemplateFile($c{dir}{bugtemplates}, "editBugTemplate.tmpl",$userid,\%record_data);
	}
	else
	{
		$tfile=&getTemplateFile($c{dir}{bugtemplates}, "viewBugTemplate.tmpl",$userid,\%record_data);
	}
}
unless ($templatefile) { $html{HEADER}[0]=&getHeader( $userid, "bug" ); }
unless ($templatefile) { $html{FOOTER}[0]=&getFooter( $userid, "bug" ); }
