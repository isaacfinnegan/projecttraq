...
my ( $TASKS, $LOGGING, $DEBUG, $userid, $group, $forcemail );

$TASKS = 1;

&startLog();
$LOGGING = 5;
my ( $connection, %results, %record, $note, $autonote, %changes, $field );
my ($q) = new CGI;
...
{
	&doError(
		"This record has been edited since you accessed it.  Please click <a href='$c{url}{base}/redir.cgi?id=$record{record_id}&method=edit&queryid=$queryid'>T$record{record_id}</a> to return to record.");
}

# Check for required fields
foreach $field ( split( ',', $c{tasktraq}{requiredforedit} ) )
{
	unless ( $record{$field} )
	{
		&doError( "$c{tasktraq}{label}{$field} must be set for this record", "", $q, "RecordUpdate" );
	}
}
########################
# Dependency handling
if ( $q->param('child') )
{
	if ( $c{tasktraq}{requirecomment} && !$note )
	{
		if ( $c{tasktraq}{requirecomment} eq 'auto' )
		{
...
# Check for comment and populate note with comment if c{tasktraq}{requirecomment} eq 'auto'
	...
	&& scalar(@changedfields) )
{
	...
	{
		$changedfields[$i] = $c{tasktraq}{label}{ $changedfields[$i] };
	}
...
unless ( $q->param('suppress') )
{
	...
	$emailsetting{record}{recordtype} = 'task';
...
sub outputConfirmation
{
	...
	if ( $pref eq "comp_confirm" )
	{
		...
		print $q->redirect("../actioncomplete.cgi?action=Edit&id=$id&result=Success&type=task&queryid=$queryid");
	}
	elsif ( $pref eq "comp_gotonext" )
	{
		if ($next)
		{
			print $q->redirect("./enterTaskForm.cgi?type=edit&id=$next&queryid=$queryid");
		}
		else
		{
			print $q->redirect("./enterTaskForm.cgi?type=edit&id=$record{'record_id'}&queryid=$queryid");
		}
