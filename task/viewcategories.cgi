...
#  my($sql) = "select * from traq_components cmp, user_groups grp, acl_traq_components acl where cmp.projectid=$projectid and grp.userid=$userid and grp.groupid=acl.groupid and cmp.componentid=acl.componentid and cmp.tasks=1";
#  %res = &doSql($sql);
%res=getComponents($projectid,'','task');
 ...
 my($templatefile)=&getTemplateFile($c{dir}{tasktemplates},"viewcomponents.tmpl",$userid);
