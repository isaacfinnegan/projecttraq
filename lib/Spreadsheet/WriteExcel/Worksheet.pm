...
sub _close {
	...
	$self->_store_conditional_formatting();
    $self->_store_eof();
}
...
#
# Store the CondFmt and CF Record data
#
sub set_conditional_formatting {
    my $self = shift;
    $self->{_conditional_formatting} = shift;
	my $new_row_count = shift || 0;
	#debug("Set Conditional Formatting to $self->{_conditional_formatting}");
	$self->{_conditional_formatting_new_row_count} = $new_row_count;
}


###############################################################################
#
# _store_conditional_formatting()
#
# Writes Excel Conditional Formatting: CondFmt + CF rows
#
sub _store_conditional_formatting {
	my $sub_debug = 0;
    my $self = shift;
	my $conditional_formatting_ptr = $self->{_conditional_formatting};
	debug("conditional_formatting_ptr=$conditional_formatting_ptr") if $sub_debug;
	return unless $conditional_formatting_ptr;

    my $cond_fmt_record_id	= 0x01B0;
    my $cf_record_id		= 0x01B1;

	my @conditional_formattings = @{$conditional_formatting_ptr};
	debug("conditional_formattings=@conditional_formattings") if $sub_debug;
	foreach my $conditional_formatting (@conditional_formattings) {
		$conditional_formatting->increment_row_count($self->{_conditional_formatting_new_row_count});
		my $cond_fmt_buffer = $conditional_formatting->generate_binary();
    	my $header = pack("vv", $cond_fmt_record_id, length($cond_fmt_buffer));
	   	$self->_append($header, $cond_fmt_buffer);
		debug(sprintf("Appended header=0X%s, cond_fmt=0X%s", unpack("H*", $header), unpack("H*", $cond_fmt_buffer))) if $sub_debug;
		if ($conditional_formatting->{cfs}) {
			my @cfs = @{$conditional_formatting->{cfs}};
			debug("cfs=@cfs") if $sub_debug;
			foreach my $cf (@cfs) {
    			my $header  = pack("vv", $cf_record_id, length($cf));
	    		$self->_append($header, $cf);
				debug(sprintf("Appended header=0X%s, cf=0X%s", unpack("H*", $header), unpack("H*", $cf))) if $sub_debug;
			}
		}
	}
}
